...
class GroupsAndResourceViewController extends Controller {
        public function index() {
                ...
                $groups_info = DB::table('group_data_inserts')
                        ...
                        ->select('group_data_inserts.name', 'group_data_inserts.description', 'group_data_inserts.image', 'group_data_inserts.category')
                        ->where('store_answers.user_email', '=', [Auth::user()->email])
                        ->get();

                $category = DB::table('group_data_inserts')
                        ->join('store_answers', 'group_data_inserts.category', '=', 'store_answers.category')
                        ->where('store_answers.user_email', [Auth::user()->email])
                        ->select('group_data_inserts.category')
                        ->distinct()->get();
                ...
                if(count($data) == 0){
                        ...
                        return view('user_profile',['resources'=>$resources, 'category'=>$category, 'groups_info'=>$groups_info]);
                }
        }
}